"""
    TightBindingCandidateSet{D}

A structure for storing information about a set of tight-binding candidates for the photonic
crystal.

## Fields
- `longitudinal :: CompositeBandRep{D}`: the longitudinal band representation `nᴸ`
- `apolarv :: Vector{CompositeBandRep{D}}`: a vector of apolar band representations
  `nᵀ⁺ᴸ[i]` over `i ∈ eachindex(apolarv)`
- `ps :: Vector{Vector{Int64}}`: a vector of vectors of coefficients `ps[i]` associated to
...
"""
struct TightBindingCandidateSet{D} <: AbstractVector{CompositeBandRep{D}}
    longitudinal::CompositeBandRep{D}    # `nᴸ`
    apolarv::Vector{CompositeBandRep{D}} # `nᵀ⁺ᴸ[i]` over `i ∈ eachindex(apolarv)`
    ps::Vector{Vector{Int64}}          # `ps[i]` associates to `nᵀ⁺ᴸ[i]`; w/ nᵀ(ω=0) = n_fixed + Q*p
end
